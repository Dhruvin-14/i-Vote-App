...
class VoterNdNominieeProfile extends StatefulWidget {
  ...
  VoterNdNominieeProfile(
      {this.name, @required this.code, this.userID, this.history});
...
class _VoterNdNominieeProfileState extends State<VoterNdNominieeProfile> {
  ...
  checkInternet() async {
    ...
    if (connectivityResult == ConnectivityResult.none) {
      ...
      setState(() {
        hasInternet = true;
        Toast.show("You are up to date", context,
            duration: Toast.LENGTH_SHORT, gravity: Toast.BOTTOM);
      });
    }
  }
  ...
  int getVotedCount() {
    DatabaseReference voted = FirebaseDatabase.instance
        .reference()
        .child("AllUsers")
        .child(code)
        .child("Voted");
  ...
  void checkElectionStatus() {
    DatabaseReference getStatus = FirebaseDatabase.instance
        .reference()
        .child("AllUsers")
        .child(code)
        .child("Status");
    getStatus.onValue.listen((event) {
      setState(() {
        ...
        if (status == "false") {
          ...
          Toast.show("Election has been Started", context,
          ...
          Toast.show("Election is now Over", context,
              duration: Toast.LENGTH_LONG, gravity: Toast.BOTTOM);
  ...
  void checkWinnerSetting() {
    DatabaseReference getWinnerSetting = FirebaseDatabase.instance
        .reference()
        .child("AllUsers")
        .child(code)
        .child("WinnerSetting");
    getWinnerSetting.once().then((DataSnapshot snapshot) {
      setState(() {
        ...
        if (winnerSetting == "VOTERS" || winnerSetting == "ALL MEMBERS") {
          ...
          DatabaseReference getWinnerName = FirebaseDatabase.instance
              .reference()
              .child("AllUsers")
              .child(code)
              .child("Winner");
          ...
          DatabaseReference checkForTie = FirebaseDatabase.instance
              .reference()
              .child("AllUsers")
              .child(code)
              .child("WinnerStatus");
          checkForTie.once().then((DataSnapshot snapshot) {
            if (snapshot.value == "Tie") {
              getWinnerName.once().then((DataSnapshot snapshot) {
                showDialogue("Hey $name",
                    "There's a tie between \n" + snapshot.value, "OKAY ;)");
              });
            } else if (snapshot.value == "NotAtie") {
              getWinnerName.once().then((DataSnapshot snapshot) {
                showDialogue("Hey $name",
                    snapshot.value + " has WON the Election", "COOL ;)");
              });
            }
          });
        } else {
          showDialogue(
              "Hey $name",
              "According to the Head's Privacy settings, you are not allowed to view the Winner..!",
              "OKAY :(");
        }
      });
    });
  ...
  void initState() {
    ...
    getElectionName = FirebaseDatabase.instance
        .reference()
        .child("AllUsers")
        .child(code)
        .child("ElectionName");
    getElectionName.once().then((DataSnapshot snapshot) {
      ...
      // register in History
      DatabaseReference history = FirebaseDatabase.instance
          .reference()
          .child("History")
          .child(userID)
          .child("VoterHistory");
                ...
                children: <Widget>[
                  ...
                  Padding(
                    ...
                    child: TextButton(
                      ...
                      style: ButtonStyle(backgroundColor: MaterialStateProperty.all(Color(0xFF09224A)),
                      ...
                      children: <Widget>[
                        ColorFiltered(
                          ...
                          colorFilter: ColorFilter.mode(
                              Colors.blueGrey[900], BlendMode.color),
                        ),
                        Align(
                                ...
                                GestureDetector(
                                  onTap: () {
                                    if (widget.history == true) {
                                      Toast.show(
                                          "You no longer have access to Chats...!",
                                          context,
                        ...
                        child: Column(
                          ...
                          children: <Widget>[
                            Padding(
                              ...
                              child: Text(statusTextOne,
                                  textAlign: TextAlign.right,
                                  style: kStatusTextOne),
                            ),
                            Padding(
                              padding: EdgeInsets.symmetric(horizontal: 20.0),
                              child: Text(statusTextTwo,
                                  textAlign: TextAlign.right,
                              ...
                              children: <Widget>[
                                ElevatedButton(
                                  ...
                                  shape: MaterialStateProperty.all(RoundedRectangleBorder(
                                    borderRadius: BorderRadius.circular(10.0),
                                    side: BorderSide(
                                        color: statusYesBtnColor, width: 1.0),
                                  ),),),
                                  ...
                                  onPressed: () {
                                    if (status == "true") {
                                      ...
                                      showDialogue(
                                          "Excited humm..!",
                                          "You can now check the Winner in the Winner Section at the Bottom",
                                          "Okay");
                                    }
                                  },
                                  child: Text(
                                ...
                                ElevatedButton(
                                  ...
                                  onPressed: () {
                                    if (status == "true") {
                                      showDialogue(
                                          "Hey $name",
                                          "Make Sure you vote before the Election is Stopped",
                                          "Okay");
                                    }
                                  },
                                  child: Text(
                          ...
                          children: <Widget>[
                            ...
                            Row(
                              children: <Widget>[
                                Column(
                                  children: <Widget>[
                                    Text("$totalVoted",
                                        ...
                                        style: kVotedNumber),
                                    ElevatedButton(
                                      style: ButtonStyle(backgroundColor: MaterialStateProperty.all(Color(0xFF1DBF9D)),
                                      shape: MaterialStateProperty.all(RoundedRectangleBorder(
                                    borderRadius: BorderRadius.circular(10.0),
                                    side: BorderSide(
                                        color: statusYesBtnColor, width: 1.0),),),), 
                                      onPressed: () {
                                        if (totalVoted != 0) {
                                          ...
                                          Toast.show(
                                              "No on has Voted yet", context,
                                              duration: Toast.LENGTH_LONG,
                                              gravity: Toast.BOTTOM);
                                        }
                                      },
                                      child: Text("View", style: kViewBtnText),
                                ...
                                CircularVotedIndicator(
                                    votedPercent: votedPercent,
                                    votedPercentText: votedPercentText),
                              ],
                            ),
                          ],
                        ),
                      ...
                      child: Column(
                        ...
                        children: <Widget>[
                          Padding(
                              padding: EdgeInsets.symmetric(
                                  vertical: 8.0, horizontal: 20.0),
                              child: Text("👑 Winner", style: kWinnerText)),
                          Padding(
                            padding: EdgeInsets.symmetric(
                                ...
                                Expanded(
                                  ...
                                  child: ElevatedButton(
                                      ...
                                      if (status == "false") {
                                        ...
                                        showDialogue(
                                            "Hey $name",
                                            "Election has just Started..!Wait for it to get over..!",
                                            "OKAY");
                                      } else if (status == "done") {
                                        checkWinnerSetting();
